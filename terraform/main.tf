...
resource "aws_ecs_cluster" "tindawg" {
  name = "tindawg"
}

resource "aws_ecr_repository" "tindawg" {
  name                 = "tindawg"
...
module "webserver" {
  ...
  ecr_repository = aws_ecr_repository.tindawg.repository_url
  ecs_cluster    = aws_ecs_cluster.tindawg.id
